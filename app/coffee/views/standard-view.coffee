      ...
      .append("svg:rect")
        .each (d, i) ->
          d3.select(@).attr
            ...
            class:     "background"
            transform: "translate(0, #{(self._metricHeight*2)*i})" # a bars distances between each metric
      ...
      .append("svg:g")
        .each (gd, i) ->
          ...
          group = d3.select(@).attr
            class: gd.metric
            transform: "translate(0, #{(self._metricWidth*2)*i})" # a bars distances between each metric
